...
export function buildDashboardPdf({ companyName, reportDate, analysis }) {
  ...
  doc.text(`AR ${companyName}`, margin, 38);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(91, 108, 100);
  doc.text('A/R Analysis — GVT vs PVT Summary', margin, 53);
  doc.setFontSize(9);
  doc.text(`As of: ${reportDate}`, doc.internal.pageSize.getWidth() - margin, 38, { align: 'right' });

  doc.autoTable({
    startY: 66, margin: { left: margin, right: margin }, theme: 'grid',
    ...
    styles: { font: 'helvetica', fontSize: 8, cellPadding: 4 }, headStyles: { fillColor: [BRAND.r, BRAND.g, BRAND.b], fontSize: 8 },
    columnStyles: { 0: { cellWidth: 65 }, 1: { halign: 'right' }, 2: { halign: 'right', cellWidth: 45 }, 3: { halign: 'right' }, 4: { halign: 'right' } },
  });

  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(15, 61, 46);
  doc.text('GVT — Customer-wise MOFT Analysis', margin, doc.lastAutoTable.finalY + 20);
  doc.autoTable({
    startY: doc.lastAutoTable.finalY + 26, margin: { left: margin, right: margin }, theme: 'grid',
    head: [['#', 'Customer', 'Not Sub.\nto MOFT', '%', 'Pmt Pending\nfrom MOFT', '%', 'Paid &\nPending', 'Total\nOutstanding']],
    body: [
    ...
    styles: { font: 'helvetica', fontSize: 6.5, cellPadding: 3, overflow: 'linebreak' }, headStyles: { fillColor: [10, 95, 168], fontSize: 6.5 },
    columnStyles: { 0: { cellWidth: 16 }, 1: { cellWidth: 138 }, 2: { halign: 'right', cellWidth: 58 }, 3: { halign: 'right', cellWidth: 26 }, 4: { halign: 'right', cellWidth: 62 }, 5: { halign: 'right', cellWidth: 26 }, 6: { halign: 'right', cellWidth: 58 }, 7: { halign: 'right', cellWidth: 62 } },
  });

  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(15, 61, 46);
  doc.text('PVT — Customer-wise Analysis', margin, doc.lastAutoTable.finalY + 20);
  doc.autoTable({
    startY: doc.lastAutoTable.finalY + 26, margin: { left: margin, right: margin }, theme: 'grid',
    ...
    columnStyles: { 0: { cellWidth: 20 }, 1: { cellWidth: 220 }, 2: { halign: 'right' }, 3: { halign: 'right' }, 4: { halign: 'right' } },
  });

  doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(15, 61, 46);
  doc.text('Semi-GVT — Customer-wise Analysis', margin, doc.lastAutoTable.finalY + 20);
  doc.autoTable({
    startY: doc.lastAutoTable.finalY + 26, margin: { left: margin, right: margin }, theme: 'grid',
    head: [['#', 'Customer', 'Outstanding - QB', 'Paid & Pending', 'Net Outstanding']],
    body: [
