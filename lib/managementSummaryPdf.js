...
export function buildManagementSummaryPdf(data) {
  const { companyName, reportDate, sectors, buckets, critical, comparison, criticalDaysThreshold, narrative, aiGenerated } = data;
  ...
  doc.setFontSize(9);
  doc.text(`As of: ${reportDate}`, pageW - margin, 40, { align: 'right' });
  doc.text(`Generated: ${new Date().toLocaleString('en-GB')}`, pageW - margin, 52, { align: 'right' });
