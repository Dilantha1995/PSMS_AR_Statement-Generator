...
const MAX_FOOTER_H = 70; // points — same idea for the footer graphic
...
export async function buildManagementSummaryPdf(data) {
  const { companyName, reportDate, referenceNumber, generatedByName, logo, letterhead, letterheadFooter, sectors, buckets, critical, comparison, criticalDaysThreshold, narrative, aiGenerated } = data;
  ...
  // Footer graphic (bottom of every page) — independent of whether the top used a letterhead or plain header.
  let footerImg = null, footerRenderH = 0;
  if (letterheadFooter) {
    try {
      const img = await loadImageDims(letterheadFooter.base64, letterheadFooter.mime);
      let fw = pageW, fh = (img.h / img.w) * fw;
      if (fh > MAX_FOOTER_H) { fh = MAX_FOOTER_H; fw = (img.w / img.h) * fh; }
      footerImg = { ...img, renderW: fw, renderH: fh };
      footerRenderH = fh;
    } catch (e) { console.warn('Letterhead footer failed to load, omitting it:', e.message); }
  }
  ...
  // repeated mini-header, and space at the bottom for the footer graphic (if any), so tables
  // never render underneath either.
  const bottomReserve = footerImg ? footerRenderH + 26 : 40;
  const tableMargin = { left: margin, right: margin, top: TOP_MARGIN_WITH_HEADER, bottom: bottomReserve };
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    if (footerImg) {
      const fx = (pageW - footerImg.renderW) / 2;
      const textY = pageH - footerImg.renderH - 6;
      doc.setFontSize(7.5); doc.setTextColor(150, 160, 155); doc.setFont('helvetica', 'normal');
      doc.text(`${companyName}  •  Management Summary`, margin, textY);
      doc.text(`Page ${p} of ${pageCount}`, pageW - margin, textY, { align: 'right' });
      doc.addImage(footerImg.dataUrl, 'PNG', fx, pageH - footerImg.renderH, footerImg.renderW, footerImg.renderH);
    } else {
      doc.setFontSize(7.5); doc.setTextColor(150, 160, 155); doc.setFont('helvetica', 'normal');
