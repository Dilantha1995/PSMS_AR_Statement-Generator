...
export async function buildManagementSummaryPdf(data) {
  const { companyName, reportDate, referenceNumber, logo, letterhead, sectors, buckets, critical, comparison, criticalDaysThreshold, narrative, aiGenerated } = data;
  const doc = new jsPDF({ orientation: 'portrait', unit: 'pt', format: 'a4' });
  const margin = 32, pageW = doc.internal.pageSize.getWidth();
  let y;

  if (letterhead) {
    const img = await loadImageDims(letterhead.base64, letterhead.mime);
    const w = pageW - margin * 2;
    const h = (img.h / img.w) * w;
    doc.addImage(img.dataUrl, 'PNG', margin, 20, w, h);
    y = 20 + h + 18;
    ...
    if (logo) {
      const img = await loadImageDims(logo.base64, logo.mime);
      const logoH = 34, logoW = (img.w / img.h) * logoH;
      doc.addImage(img.dataUrl, 'PNG', margin, 16, logoW, logoH);
  ...
  metaParts.push(`Generated: ${new Date().toLocaleString('en-GB')}`);
  ...
  doc.autoTable({
    ...
    head: [['Sector', 'Total Outstanding - QB', '%', 'Paid & Pending', 'Net Outstanding']],
    body: [
  ...
  doc.autoTable({
    ...
    head: [['Ageing Bucket', 'Invoice Count', 'Open Balance']],
  ...
  if (critical.length) {
    doc.autoTable({
      startY: doc.lastAutoTable.finalY + 26, margin: { left: margin, right: margin }, theme: 'grid',
      head: [['Customer', 'Invoice #', 'Due Date', 'Days Overdue', 'Open Balance']],
